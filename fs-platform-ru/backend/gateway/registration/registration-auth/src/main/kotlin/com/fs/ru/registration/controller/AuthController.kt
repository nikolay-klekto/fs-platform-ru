...
class AuthController() {
    ...
    fun authenticateUser(
        ...
        if (!captchaService.validateCaptcha(loginRequest.recaptchaToken!!)) {
            ...
            if (!user.enabled) {
                return ResponseEntity(
                    ResponseMessage("Account is not verified yet! Please, follow the link in the confirmation email."),
    ...
    fun registerUser(@Valid @RequestBody newUser: NewUser): ResponseEntity<*> {
        ...
        if (!captchaService.validateCaptcha(newUser.recaptchaToken!!)) {
            ...
            return ResponseEntity(
                ResponseMessage("Please, follow the link in the confirmation email to complete the registration."),
                HttpStatus.OK
            )
        } else {
