...
4. **Когда задача готова:**
    ...
    9. После этого проходит тестирование задачи:
    - Если всё работает корректно — задача переносится в колонку **готово**
    - Если найдены проблемы — в задаче оставляется комментарий и она перемещается в колонку **доработки после проверки**
      В этом случае создаётся новая ветка с тем же названием, но с префиксом `fix_test_bag_`, чтобы тестировщику было проще её повторно проверить
